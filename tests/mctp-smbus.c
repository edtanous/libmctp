...
static void rx_message(uint8_t eid, void *data, void *msg, size_t len)
{
	...
	switch((*message_data & NVME_MI_MESSAGE_TYPE_MASK)){
	case MCTP_CONTROL_MESSAGE_TYPE:
		if (len < 4){
			fprintf(stderr, "Not enough bytes for control message\n");
			return;
		}

		if (message_data[3] != MCTP_CONTROL_MSG_STATUS_SUCCESS){
			fprintf(stderr, "Control message failed with code %d\n");
			return;
		}

		if (message_data[2] == MCTP_COMMAND_CODE_SET_EID){
			// TODO(ed) check buffer limits
			fprintf(stderr, "EID is 0x%02X\n", message_data[5]);
			fprintf(stderr, "EID poll size is %d\n", message_data[6]);
			return;
		}

		fprintf(stderr, "Got an unknown control message\n");
	...
	default:
		fprintf(stderr, "got unknown type message_type=%d\n",
...
#ifdef EXAMPLE_NUM_3
	uint8_t message_buf[] = {
		MCTP_CONTROL_MESSAGE_TYPE,
		0x80, // MCTP request TODO use ROR
		MCTP_COMMAND_CODE_SET_EID,
		0,
		0xC};
	
	nv_ctx.request_type = MCTP_CONTROL_MESSAGE_TYPE;

	rc = mctp_message_tx(mctp, eid, message_buf, sizeof(message_buf));
#endif
	if (rc < 0)
		return EXIT_FAILURE;
